<template>
  <!--
    权限配置（RbacConfigView · 增强③ R5）：角色 × 资源 权限矩阵。
    功能真源 = 后端 /api/rbac。集团统一定义角色与矩阵；写操作受后端 @RequiresPermission("rbacconfig") 限制。
  -->
  <AppShell>
    <section class="view view-rbac">
      <div class="phead">
        <div><div class="kqt">{{ $t('rbac.tag') }}</div><h1>{{ $t('rbac.title') }}</h1></div>
        <div class="sp"></div>
        <button class="btn ghost sm" @click="openCreate">{{ $t('rbac.newRole') }}</button>
      </div>

      <div class="g">
        <!-- 左：角色列表 -->
        <div class="card">
          <div class="ch"><h3>{{ $t('rbac.roles') }}</h3><span class="cnt">{{ roles.length }}</span></div>
