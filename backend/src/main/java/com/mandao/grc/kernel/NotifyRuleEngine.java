...
public class NotifyRuleEngine {
    ...
    /**
     * 评估全部启用规则一轮，返回本轮新产出的通知条数。today 由外部传入，便于测试确定性。
     */
    @Transactional
        ...
        for (Object[] rule : rules) {
            ...
            try {
                ...
                produced += switch (source) {
                    case "REMEDIATION_OVERDUE" -> scanRemediationOverdue(ruleId, template, today);
                    case "ASSESSMENT_STALLED" -> scanAssessmentStalled(ruleId, template, today, days);
                    case "REG_NEW" -> scanRegNew(ruleId, template, today, Math.max(days, 1));
                    case "KRI_BREACH" -> scanKriBreach(ruleId, template);
    ...
    private int scanRemediationOverdue(long ruleId, String template, LocalDate today) {
        ...
        for (Object[] r : rows) {
            ...
            n += dispatch("REMEDIATION", ((Number) r[0]).longValue(), "RULE_REMEDIATION_OVERDUE",
                    "rule=" + ruleId, ((Number) r[1]).longValue(), msg, ruleId);
        }
        return n;
    }
    ...
    private int scanKriBreach(long ruleId, String template) {
        @SuppressWarnings("unchecked")
        List<Object[]> rows = em.createNativeQuery(
