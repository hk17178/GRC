...
public class EvidenceService {
    ...
    public Evidence upload(Long orgId, Long planId, Long findingId, Long remediationId,
        ...
        return upload(orgId, planId, findingId, remediationId, null, null, null, null,
                name, fileName, contentType, data, actor);
    }

    /** 兼容重载（七轮六参签名）：仅 filing/incident，无问询/处罚。 */
    @Transactional
    public Evidence upload(Long orgId, Long planId, Long findingId, Long remediationId,
                           Long filingId, Long incidentId,
                           String name, String fileName, String contentType, byte[] data, String actor) {
        return upload(orgId, planId, findingId, remediationId, filingId, incidentId, null, null,
                name, fileName, contentType, data, actor);
    }

    /**
     * 上传证据（七轮 7-2 + M11 B13 扩展）：关联对象扩到 报送/重大事件/问询/处罚——监管回执与
     * 答复回函、整改凭证统一入证据库，与审计证据同一套 sha256 固化与留痕，形成完整举证链。
     */
    @Transactional
    public Evidence upload(Long orgId, Long planId, Long findingId, Long remediationId,
                           Long filingId, Long incidentId, Long inquiryId, Long penaltyId,
                           String name, String fileName, String contentType, byte[] data, String actor) {
        if (planId == null && findingId == null && remediationId == null && filingId == null
                && incidentId == null && inquiryId == null && penaltyId == null) {
            throw new IllegalArgumentException("证据须至少关联 审计计划/审计发现/整改单/监管报送/重大事件/监管问询/处罚约谈 之一");
        }
        ...
        e.attachRegulatory(filingId, incidentId, inquiryId, penaltyId);
        ...
        hashChainService.append(orgId, "EVIDENCE_UPLOAD", actor, "EVIDENCE:" + saved.getId(),
                "上传证据「" + name + "」sha256=" + saved.getSha256()
                        ...
                        + (penaltyId == null ? "" : " penalty=" + penaltyId));
        return saved;
    }

    /**
     * 证据列表（七轮 7-2 + M11 B13 七维过滤 + 7-8 投影分页）：返回不含文件字节的投影，
    ...
    public List<EvidenceSummary> listSummaries(Long planId, Long findingId, Long remediationId,
        ...
        return evidenceRepo.findSummaries(planId, findingId, remediationId, filingId, incidentId,
                inquiryId, penaltyId, org.springframework.data.domain.PageRequest.of(p, s));
    }
    ...
    public void streamDossierZip(Long planId, java.io.OutputStream out) {
        ...
        List<Long> evidenceIds = evidenceRepo.findSummaries(null, null, null, null, null, null, null,
