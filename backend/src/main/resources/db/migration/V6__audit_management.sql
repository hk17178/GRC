...
--        · status：审计计划生命周期状态机 PLANNED→IN_PROGRESS→REPORTED→CLOSED；
...
--   3) 新表 audit_finding（审计发现）：severity 五级口径(LOW/MID/HIGH/CRITICAL，自定四档严重度)，
...
-- 审计计划生命周期状态机：PLANNED → IN_PROGRESS → REPORTED → CLOSED
...
-- severity 严重度：LOW / MID / HIGH / CRITICAL（审计发现四档严重度）
...
--   单向推进，不允许跳级、不允许逆向；唯 CONFIRMED_CLOSED 算外审闭环（由 AuditFindingService 强校验）。
CREATE TABLE audit_finding (
  ...
  severity                  VARCHAR(12),                                       -- 严重度（LOW/MID/HIGH/CRITICAL）
